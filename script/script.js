...
function setupUI() {
  ...
  try {
    ...
    xcoord.onkeypress = checkInput;
    xcoord.addEventListener("input", inputUpdate);
    ycoord.onkeypress = checkInput;
    ycoord.addEventListener("input", inputUpdate);
...
function loadFromQuery() {
  ...
  if (data) {
    for (let i = 0; i < data.length; i++) {
      ...
      tempBuffer += `${line[0] + lastFrame} ${key.join(
        ";"
      )} ${line[2].replaceAll("_", ";")} ${line[3].replaceAll("_", ";")}\n`;
      lastFrame += line[0];
    }
...
function createQuery() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    line[0] = Number(line[0]);
    tempBuffer += `${line[0] - lastFrame}.${key.join("_")}.${line[2].join(
      "_"
    )}.${line[3].join("_")}~`;
    lastFrame = line[0];
  }
  ...
  parameters.push(
    encodeURIComponent("buffer") + "=" + encodeURIComponent(tempBuffer)
  );
...
function paste(frame, data = pasteboard) {
  ...
  if (
    possibleFrame ==
      Number(tempBuffer[tempBuffer.length - 1].split(" ")[0]) + 1 ||
    (tempBuffer.length == 1 && tempBuffer[0] == "")
  ) {
...
function newFrame() {
  ...
  if (apparatus.children.length > 1) {
    frame = Number(
      apparatus.children[apparatus.children.length - 1].children[0].innerHTML
    );
  }
...
function downloadBuffer() {
  /* Although the function name implies it only downloads one buffer,
    recent updates have changed its functionality to include the entire buffer array */
  for (let i = 0; i < bufferArray.length; i++) {
    let elem = document.createElement("a");
    elem.setAttribute(
      "href",
      "data:text/plain;charset=utf-8," + encodeURIComponent(bufferArray[i])
    );
...
function populateTable(newFile = false) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Sticks
    let stick1 = line[2].split(";").map(Number);
    let stick2 = line[3].split(";").map(Number);
    let lstick = document.createElement("td");
    let rstick = document.createElement("td");
    lstick.className = "lstick";
    rstick.className = "rstick";
    lstick.innerHTML = stick1;
    rstick.innerHTML = stick2;
    lstick.title = stick1;
    rstick.title = stick2;
    row.appendChild(lstick);
    row.appendChild(rstick);
...
function fillText(sec = 0) {
  ...
  textarea.innerHTML =
    "<code>" +
    bufferArray[bufferIndex].replaceAll("\n", "<br>") +
    `</code><br><br>${tag}`;
}
...
function cursorMove(e) {
  ...
  let x =
    ((clientX - joystick.offsetLeft - joystick.offsetWidth / 2) * radius) /
    (joystick.offsetWidth / 2);
  let y =
    ((clientY - joystick.offsetTop - joystick.offsetWidth / 2) * -1 * radius) /
    (joystick.offsetWidth / 2);
  x = x.toFixed(0);
  y = y.toFixed(0);
...
function inputUpdate(e = null) {
  ...
  let clientX =
    (x * (joystick.offsetWidth / 2)) / radius +
    joystick.offsetWidth / 2 +
    joystick.offsetLeft;
  let clientY =
    (y * (joystick.offsetWidth / 2)) / radius / -1 +
    joystick.offsetWidth / 2 +
    joystick.offsetTop;
  if (!checkBounds(x, y, radius)) {
    coords[0].value = 0;
...
function changeStick(row, col) {
  ...
  let [x, y] = apparatus.children[row].children[col].innerHTML
    .split(",")
    .map(Number);
...
function importBuffer() {
  ...
  let response = window.prompt(
    "Which script should be imported?",
    bufferTitles[defaultN]
  );
  if (!response) {
    return;
  }
  ...
  if (
    response == bufferTitles[bufferIndex] &&
    !window.confirm("Are you sure you want to import a script into itself?")
  ) {
    return;
  }
  ...
  frame = Number(frame);
  let prompt = window.prompt(
    "After which frame should this script be imported?",
    frame
  );
  if (prompt) {
    frame = Number(prompt);
